...
type PeerQueue interface {
	AddBroadcastWantHaves([]bswl.WantKey)
	AddWants([]bswl.WantKey, []bswl.WantKey)
	AddCancels([]bswl.WantKey)
	ResponseReceived(ks []bswl.WantKey)
	Startup()
	Shutdown()
}
...
func (pm *PeerManager) ResponseReceived(p peer.ID, ks []bswl.WantKey) {
	pm.pqLk.Lock()
	pq, ok := pm.peerQueues[p]
	pm.pqLk.Unlock()
...
// the peer.
func (pm *PeerManager) BroadcastWantHaves(ctx context.Context, wantHaves []bswl.WantKey) {
	pm.pqLk.Lock()
	defer pm.pqLk.Unlock()
...
func (pm *PeerManager) SendWants(ctx context.Context, p peer.ID, wantBlocks []bswl.WantKey, wantHaves []bswl.WantKey) {
	pm.pqLk.Lock()
	defer pm.pqLk.Unlock()
...
func (pm *PeerManager) SendCancels(ctx context.Context, cancelKs []bswl.WantKey) {
	pm.pqLk.Lock()
	defer pm.pqLk.Unlock()
...
func (pm *PeerManager) CurrentWants() []bswl.WantKey {
	pm.pqLk.RLock()
	defer pm.pqLk.RUnlock()
...
func (pm *PeerManager) CurrentWantBlocks() []bswl.WantKey {
	pm.pqLk.RLock()
	defer pm.pqLk.RUnlock()
...
func (pm *PeerManager) CurrentWantHaves() []bswl.WantKey {
	pm.pqLk.RLock()
	defer pm.pqLk.RUnlock()
