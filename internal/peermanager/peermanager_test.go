...
func (fp *mockPeerQueue) AddBroadcastWantHaves(whs []wl.WantKey) {
	fp.msgs <- msg{fp.p, nil, whs, nil}
}
func (fp *mockPeerQueue) AddWants(wbs []wl.WantKey, whs []wl.WantKey) {
	fp.msgs <- msg{fp.p, wbs, whs, nil}
}
func (fp *mockPeerQueue) AddCancels(cs []wl.WantKey) {
	fp.msgs <- msg{fp.p, nil, nil, cs}
}
func (fp *mockPeerQueue) ResponseReceived(ks []wl.WantKey) {
}
...
func TestBroadcastWantHaves(t *testing.T) {
	...
	// first peer
	peerManager.BroadcastWantHaves(ctx, []wl.WantKey{cids[0], cids[2]})
	collected = collectMessages(msgs, 2*time.Millisecond)
...
func TestSendWants(t *testing.T) {
	...
	peerManager.Connected(peer1)
	peerManager.SendWants(ctx, peer1, []wl.WantKey{cids[0]}, []wl.WantKey{cids[2]})
	collected := collectMessages(msgs, 2*time.Millisecond)

	if len(collected[peer1].wantHaves) != 1 || collected[peer1].wantHaves[0] != cids[2] {
		t.Fatal("Expected want-have to be sent to peer")
	}
	if len(collected[peer1].wantBlocks) != 1 || collected[peer1].wantBlocks[0] != cids[0] {
		t.Fatal("Expected want-block to be sent to peer")
	}

	// new wantblock: cids[1]; new wanthave: cids[3]
	peerManager.SendWants(ctx, peer1, []wl.WantKey{cids[0], cids[1]}, []wl.WantKey{cids[2], cids[3]})
	collected = collectMessages(msgs, 2*time.Millisecond)

	// First want-have and want-block should be filtered (because they were
	// already sent)
	if len(collected[peer1].wantHaves) != 1 || collected[peer1].wantHaves[0] != cids[3] {
		t.Fatal("Expected want-have to be sent to peer")
	}
	if len(collected[peer1].wantBlocks) != 1 || collected[peer1].wantBlocks[0] != cids[1] {
		t.Fatal("Expected want-block to be sent to peer")
	}
}
...
func TestSendCancels(t *testing.T) {
	...
	peerManager.SendWants(ctx, peer1, []wl.WantKey{cids[0], cids[1]}, []wl.WantKey{cids[2]})

	// Clear messages
	collectMessages(msgs, 2*time.Millisecond)

	// Send cancels for 1 want-block and 1 want-have
	peerManager.SendCancels(ctx, []wl.WantKey{cids[0], cids[2]})
	collected := collectMessages(msgs, 2*time.Millisecond)
...
func (*benchPeerQueue) AddBroadcastWantHaves(whs []wl.WantKey)      {}
func (*benchPeerQueue) AddWants(wbs []wl.WantKey, whs []wl.WantKey) {}
func (*benchPeerQueue) AddCancels(cs []wl.WantKey)                  {}
func (*benchPeerQueue) ResponseReceived(ks []wl.WantKey)            {}
...
func BenchmarkPeerManager(b *testing.B) {
	...
	for n := 0; n < b.N; n++ {
		...
		if r == 0 {
			...
			wants := testutil.GenerateWantKeys(30)
