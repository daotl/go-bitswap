...
func (sw *sentWants) wantHavesKeys() []cid.Cid {
	sw.Lock()
	defer sw.Unlock()
	return sw.wantHaves.Cids()
}
func (sw *sentWants) wantBlocksKeys() []cid.Cid {
	sw.Lock()
	defer sw.Unlock()
	return sw.wantBlocks.Cids()
}
...
func TestSendWants(t *testing.T) {
	...
	if !testutil.MatchCidsIgnoreOrder(sw.wantBlocksKeys(), blkCids0) {
		t.Fatal("Wrong keys")
	}
...
func TestSendsWantBlockToOnePeerOnly(t *testing.T) {
	...
	if !testutil.MatchCidsIgnoreOrder(sw.wantHavesKeys(), blkCids0) {
		t.Fatal("Wrong keys")
	}
}
...
func TestReceiveBlock(t *testing.T) {
	...
	if !testutil.MatchCidsIgnoreOrder(sw.wantBlocksKeys(), cids) {
		t.Fatal("Wrong keys")
	}
	...
	bpm.ReceiveFrom(peerA, nil, []wl.WantKey{wl.NewWantKey(cids[0], "")})
	spm.Update(peerA, []cid.Cid{cids[0]}, []cid.Cid{}, []cid.Cid{cids[1]})
	// peerB: HAVE cid0, cid1
	bpm.ReceiveFrom(peerB, wl.CidsToKeys(cids, ""), nil)
...
func TestCancelWants(t *testing.T) {
	...
	sent := swc.cancelled()
	if !testutil.MatchCidsIgnoreOrder(wl.KeysToCids(sent), cancelCids) {
		t.Fatal("Wrong keys")
	}
}
